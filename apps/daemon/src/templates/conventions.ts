...
### You can't 'finish' a Godot project — the user must trigger asset import

Godot 4 requires every asset to have a \`.import\` sidecar file generated by
the Godot Editor. When you create a new PNG / WAV / FBX with \`image_gen\` /
\`generate2dsprite\` / \`generate2dmap\`, the file lands on disk but **the
.import sidecar is NOT created automatically** — only the Godot Editor's
filesystem dock generates it.

Without the sidecar, \`load("res://path/to.png")\` fails at runtime with:

\`\`\`
ERROR: No loader found for resource: res://assets/props/foo.png
\`\`\`

You can't trigger the import yourself — there's no CLI flag for it. So
after generating any new asset(s), END THE TURN with an explicit
'next step for the user':

> ✋ **Action needed**: I added new assets under \`assets/\`. Open the
> project in Godot Editor once so it generates the \`.import\` sidecars,
> then close it. After that, OGF Play (or F5 in the editor) will work.
> Until you do this, \`load()\` will fail with 'No loader found'.

DO NOT say 'the project is ready to play' or 'you can run it now' if
you generated new assets in this turn. The user needs the Godot Editor
import step BEFORE that becomes true.

This applies to every new asset, every turn. Once the user has imported
a particular file, the .import sidecar persists (commit them to git);
later edits to the same file are auto-detected by the editor on next
open.
